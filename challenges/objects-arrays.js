...
// Given an array of college (graduates <- array name).  Complete the following requests using any array method you like

/* Request 1: Create a new array called universities that contains all the universities in 
the graduates array. This will be an array of strings.

Once you have the new array created, log the result. */
const universities = [];

graduates.forEach((x) => universities.push(x.university));
...
graduates.forEach((x) => contactInfo.push(`${x.first_name} ${x.email}`));
...
The zoos want to display both the scientific name and the animal name in front of the habitats. Populate 
the displayNames array with only the animal_name and scientific_name of each animal. displayNames will be 
...
create a new array of strings named lowCaseAnimalNames, each string following this pattern: "jackal, asiatic". 
Log the resut.

*/

const lowCaseAnimalNames = zooAnimals.map((names) =>
  names.animal_name.toLowerCase()
);
...
array of objects called lowPopulationAnimals which contains only the animals with a population less than 5.
